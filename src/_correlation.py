...
def cluster_correlation_search(
	...
	edges_positive = {(n2i[i], n2i[j], G[i][j]["weight"]) for (i, j) in G.edges() if G[i][j]["weight"] >= 0.0}
	...
	solutions = pool.starmap(
		...
		[(n, classes, G.nodes(), init_state, max_attempts, max_iters) for n in range(2, max_senses)], )
	pool.close()
	# print(solutions[0])
