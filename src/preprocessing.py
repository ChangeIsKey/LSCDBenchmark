...
class ContextPreprocessor(BaseModel, ABC):
	...
	def __call__(
		...
		return Series(
			{
				"context_preprocessed": context, "target_index_begin": start, "target_index_end": end,
			}
		)
...
class Lemmatize(ContextPreprocessor):
	def preprocess_context(
		...
		idx: int = s.get(
			key="indexes_target_token_lemmatized", default=s.indexes_target_token_tokenized
		)
		start, end = self.character_indices(
