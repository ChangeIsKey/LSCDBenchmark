...
class Target(BaseModel):
    ...
    [self._uses_df, self._uses_df.apply(self.preprocessing.__call__, axis=1), ], axis=1, )
    ...
    {
    "identifier": Column(dtype=str, unique=True), "grouping": Column(dtype=str),
    "context_preprocessed": Column(str), "target_index_begin": Column(int), "target_index_end": Column(int),
    }
    )
    ...
    @property
    def augmented_judgments_df(
    self
    ) -> DataFrame:
    if self._augmented_judgments_df is None:
    self._augmented_judgments_df = pd.merge(
    self.judgments_df, self.uses_df, left_on="identifier1", right_on="identifier", how="left", )
    self._augmented_judgments_df = pd.merge(
    self._augmented_judgments_df, self.uses_df, left_on="identifier2", right_on="identifier", how="left", )
    return self._augmented_judgments_df

    @property
    def judgments_schema(
    self
    ) -> DataFrameSchema:
    return DataFrameSchema(
    {
    "identifier1": Column(dtype=str), "identifier2": Column(dtype=str),
    ...
    n: int | None = None,
    replace: bool | None = None, ) -> list[tuple[Use, Use]]:
    ...
    self,
    pairing: Literal["COMPARE", "EARLIER", "LATER"], ) -> tuple[list[UseID], list[UseID]]:
    match pairing:
    case "COMPARE":
    ...
    (self.augmented_judgments_df.grouping_x == group_0) & (self.augmented_judgments_df.grouping_y == group_1)]

    return (judgments.identifier1.tolist(), judgments.identifier2.tolist(),)
