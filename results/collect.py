...
def collect_results() -> pd.DataFrame:
    ...
    for day in tqdm(
        list(outputs.iterdir()), desc="Processing days (outputs folder)", leave=False
    ):
        for experiment in tqdm(
            list(day.iterdir()), desc="Processing experiments", leave=False
        ):
            date_time = datetime.strptime(
                f"{day.name} {experiment.name}", "%Y-%m-%d %H-%M-%S"
            )
            results = process_experiment(experiment, date_time, results, datasets)

    for day in tqdm(
        multirun.iterdir(), desc="Processing days (multirun folder)", leave=False
    ):
        for time in tqdm(day.iterdir(), desc="Processing launch times", leave=False):
            for experiment in tqdm(
                time.iterdir(), desc="Processing experiments", leave=False
            ):
                if experiment.is_dir():
                    date_time = datetime.strptime(
                        f"{day.name} {time.name}", "%Y-%m-%d %H-%M-%S"
                    )
                    results = process_experiment(
                        experiment, date_time, results, datasets
                    )
    ...
    cols = first_cols + [
        col for col in results.columns.tolist() if col not in first_cols
    ]
...
def process_experiment(
    ...
    try:
        score = np.loadtxt(experiment / "score.txt").item()
        if score == np.nan:
            return results
        ...
        predictions = pd.read_csv(
            experiment / "predictions.tsv", sep="\t", engine="pyarrow"
        )
        ...
        row = pd.concat(
            [
                ...
                pd.DataFrame(
                    [{"time": date_time, "score": score, "n_targets": n_targets}]
                ),
            ],
            axis=1,
        )
